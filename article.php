...
<body>
  ...
  <div id="myCarousel" class="carousel slide" data-ride="carousel" data-interval="false">
    ...
    <div class="item active">
      ...
      <?php 
    //   $image_url = "https://1000wordphilosophy.files.wordpress.com/2021/05/happiness.jpg?w=640";
    //        $url_parsed_arr = parse_url($image_url);
    //           print_r($url_parsed_arr);
      function validateMediaType($link) {
        ...
        foreach ($videoArr as $token) {
            ...
            //   if (preg_match('/\.(jpeg|jpg|png|gif)$/i', $link)) {
            //     // Successful match!
            //         return "image";
            //     } else if(preg_match('/\.(mp4|3gp|ogg)$/i', $link)){
...
<script type="text/javascript">
    ...
    function next(){
        ...
        // console.log($('.myCarousel-target.active').attr('data-slide-to'));
...
<div style=" padding:8px; margin:8px;">
    <span style="font-size: 16px;" class="sub_title">Related Articles</span>
    <div style="display:flex; overflow-x: scroll;" >
    
        <?php foreach($arr2 as $key => $val){ ?>
            <?php if($val['id'] != $articleId){ ?>
            <img style="width: 40%; margin-right:4px;" src="<?=$val['pic']?>" alt="" onclick="toArticlePg(<?=$val['id']?>)" >
            <?php } ?>
        <?php } ?>
    </div>
</div>
